...
class MazeGenerator:
    def __init__(
        ...
        self._seed = seed
        self._entryx = entry_cell[0] if 0 <= entry_cell[0] < self._width else 0
        self._entryy = (
            entry_cell[1] if 0 <= entry_cell[1] < self._height else 0
        )
        self._exitx = (
            exit_cell[0]
            if 0 <= exit_cell[0] < self._width
            else self._width - 1
        )
        self._exity = (
            exit_cell[1]
            if 0 <= exit_cell[1] < self._height
            else self._height - 1
        )
    ...
    def _add_42_to_maze(self) -> None:
        ...
        if (
            len(ft_small) * 2 > self._height
            or len(ft_small[0]) * 2 > self._width
        ):
            ...
            if self._is_available(nx, ny):
                ...
                if (
                    ...
                    if (
                        ...
                        and (self._maze[ny][nx] & (~opp_code)) != 0
                    ):
